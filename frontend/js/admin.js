...
function renderArchive() {
    ...
    tbody.innerHTML = archivedUsers.map(u => `
        <tr>
            ...
            <td>${escapeHtml(u.fullname)}</td>
            <td><span class="role-badge role-${u.role.toLowerCase()}">${u.role}</span></td>
            <td>${u.archivedDate}</td>
            <td><button class="btn-cancel" onclick="permanentDelete(${u.id})"><i class="fas fa-trash"></i> Delete</button></td>
        </tr>
    `).join('');
}
...
function openReplyModal(inquiryId, guestName, guestEmail, guestMessage) {
    ...
    const inquiryIdField = document.getElementById('replyInquiryId');
    const toNameField = document.getElementById('replyToName');
    const toEmailField = document.getElementById('replyToEmail');
    const originalMessageField = document.getElementById('replyOriginalMessage');
    const replyMessageField = document.getElementById('replyMessage');
    
    inquiryIdField.value = inquiryId;
    toNameField.value = guestName;
    toEmailField.value = guestEmail;
    originalMessageField.value = guestMessage;
...
function sendReply() {
    ...
    const adminName = adminUser ? adminUser.name : 'HealthDesk Admin';
...
Email: clinic@healthdesk.com
    ...
    showToast(`Reply sent to ${guestEmail} (Demo - Check console)`, 'success');
    ...
    container.innerHTML = filtered.map(inquiry => `
        <div class="inquiry-card">
            ...
            <div style="display: flex; gap: 8px; margin-top: 12px;">
                ...
                ` : ''}
                <button class="btn-cancel" style="padding: 6px 12px; font-size: 12px;" onclick="deleteInquiry(${inquiry.id})"><i class="fas fa-trash"></i> Delete</button>
            </div>
        </div>
    `).join('');
}
...
function searchArchive() {
    ...
    tbody.innerHTML = filtered.map(u => `
        <tr>
            ...
            <td><span class="role-badge role-${u.role.toLowerCase()}">${u.role}</span></td>
            <td>${u.archivedDate}</td>
            <td><button class="btn-cancel" onclick="permanentDelete(${u.id})">Delete</button></td>
        </tr>
    `).join('');
}
